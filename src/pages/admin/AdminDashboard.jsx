...
function getPeriodBounds(period, offset) {
  ...
  const month = TODAY_DATE.getUTCMonth();
...
export default function AdminDashboard() {
  ...
  useEffect(() => {
    setLoading(true);
    fetchWorkRecordsByPeriod(null, null, start, end)
  ...
  const chartData = useMemo(() => {
    if (period !== 'ano') {
      ...
      for (let d = sd; d <= ed; d++) {
        ...
        const recs = periodRecs.filter(r => r.date === iso);
        days.push({
  ...
  const monthPT = MONTHS[TODAY_DATE.getUTCMonth()];
    ...
    <div className="space-y-5">
      ...
      <div className="animate-fade-up">
        <p className="text-xs capitalize" style={TM}>
          {weekday}, {TODAY_DATE.getUTCDate()} de {monthPT} de {TODAY_DATE.getUTCFullYear()}
        </p>
