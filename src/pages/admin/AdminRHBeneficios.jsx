  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: '4px' }}>
      <input type="number" value={val} onChange={e => setVal(e.target.value)}
        style={{ width: '80px', padding: '4px 8px', borderRadius: '6px', border: '1px solid #CBD5E1', fontSize: '13px' }} />
      <button onClick={() => onSave(Number(val))} style={{ background: '#DCFCE7', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: '5px', color: '#059669' }}><Check size={12} /></button>
      <button onClick={onCancel} style={{ background: '#FEE2E2', border: 'none', cursor: 'pointer', padding: '4px', borderRadius: '5px', color: '#E11D48' }}><X size={12} /></button>
    </div>
  );
}
    ...
    <div className="space-y-6">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px' }}>
        {[
          ...
          { label: 'Diária média', value: fmtCurrency(ativos.reduce((s,e) => s + (e.dailyRate||0), 0) / (ativos.length||1)) },
          { label: 'Hora extra média', value: fmtCurrency(ativos.reduce((s,e) => s + (e.overtimeRate||0), 0) / (ativos.length||1)) },
        ].map((k, i) => (
